...
async function mcqajax() {
    ...
    console.log(img);
    if(img.src == undefined || img.src == "http://localhost:3000/api/v1/saveQuizdetails"){
...
async function ajaxUpdatePoll(ele, serial_no) {
    ...
    if (img === undefined || img.src == "http://localhost:3000/api/v1/saveQuizdetails") {
        img = null;
        ...
        filename = filename[filename.length - 1];
        console.log(filename);
        let prevQ =  newElement.children[0].children[0].children[0].children[2].innerText;
        console.log(prevQ);
        console.log("prev q");
        let updateSrc = document.getElementById(`question${serial_no}${prevQ}`);
        if(updateSrc != null){
            console.log(updateSrc);
            updateSrc.removeAttribute('id');
            console.log("src update  = "+ src);
            console.log(src);
            // updateSrc.setAttribute("src", `${src.innerHTML}`);
            updateSrc.setAttribute("id",`question${serial_no}${question}`);
        }else{
            let image = document.getElementById('image1').cloneNode(true);
           ...
           console.log("image = "+image+" src= = "+src);
            ...
            filename = filename[filename.length - 1];
            console.log(filename);
            image.setAttribute('id', `question${serial_no}${question}`);
            console.log(image);
            let imageList = document.getElementById("imageName1");
...
async function fillajax() {
    ...
    if (img.src == "http://localhost:3000/api/v1/saveQuizdetails") {
...
async function editFillQ(ele) {
    ...
    if(src != undefined ){
        ...
        console.log(Qimage);
        Qimage.style.display = "block";
        let prevQ =  newElement.children[0].children[0].children[0].children[2].innerHTML;
        let updateSrc = document.getElementById(`question${serial_no}${prevQ}`);
        updateSrc.removeAttribute('id');
        console.log("src");
        console.log(src);
...
async function mcqUpdate(serial_no) {
    ...
    if (img === undefined || img.src == "http://localhost:3000/api/v1/saveQuizdetails") {
        ...
        console.log("image in pollAjax = "+img);

        src = img.src;
        console.log("src in pollAjax = "+src);
        filename = src.split('/');
        filename = filename[filename.length - 1];
        console.log(filename);
        let prevQ =  newElement.children[0].children[0].children[0].children[2].innerHTML;
        ...
        if(updateSrc){
            ...
            console.log(src);
            updateSrc.setAttribute("src", `${src.innerHTML}`);
            updateSrc.setAttribute("id",`question${serial_no}${userComment}`);
        }else{
            updateSrc.setAttribute("src", `${src.innerHTML}`);
            updateSrc.setAttribute("id",`question${serial_no}${question}`);
        }
...
async function ajaxUpdatefill(ele, serial_no) {
    ...
    newElement = document.getElementById(`question${serial_no}`);
